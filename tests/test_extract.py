...
def test_extract_from_unstructured_text(mock_cfg, mock_text_response):
    text = (
        "We are looking for qualified Uber Drivers in Cape Town. "
    ...
    with (
        patch("llmcall.extract.get_config", return_value=mock_cfg),
        patch(
            "llmcall.extract.completion",
            return_value=mock_text_response(
                '{"title":"Uber Driver","location":"Cape Town",'
                '"contact":"021 123 4567","salary":"R5000 per month"}'
            ),
        ),
    ):
